...
template <typename A>
struct is_visitable_container_t<A, std::void_t<typename remove_cvref_t<A>::iterator>>
        : std::bool_constant<is_visitable_element<typename remove_cvref_t<A>::iterator::value_type>> {};
...
T& inferVisitArgType(std::function<R(T&)>);

template <typename F>
typename lambda_traits<F>::template arg<0>& inferVisitArgType(F const&);

template <typename F>
using infer_visit_arg_type = std::remove_reference_t<decltype(inferVisitArgType(std::declval<F>()))>;

template <typename F>
using infer_visit_node_type = copy_const<infer_visit_arg_type<F>, visit_root_type<infer_visit_arg_type<F>>>;
...
auto makeLambdaVisitor(F&& f) {
    static_assert(std::is_same_v<CastType, void> || std::is_same_v<CastType, AllowCrossCast>);
    using Root = copy_const<std::remove_reference_t<Node>, visit_root_type<Node>>;
    return LambdaVisitor<std::decay_t<F>, Root, CastType>(std::forward<F>(f));
}
...
std::enable_if_t<detail::is_visitor<Visitor>> visit(CC&& xs, Visitor&& visitor, Args&&... args) {
    if constexpr (detail::is_visitable<CC>) {
        ...
        static_assert(is_range_v<remove_cvref_t<CC>>, "don't know how to traverse the root argument");
...
std::enable_if_t<!detail::is_visitor<F>> visit(CC&& root, F&& fun) {
    ...
    souffle::visit(
            root, detail::makeLambdaVisitor<CastType, std::remove_reference_t<F>, copy_const<Arg, Node>>(
                          std::forward<F>(fun)));
}
