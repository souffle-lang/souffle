...
private:
    ...
    std::unique_ptr<tree_node> explainLabel(std::string relName, plabel label, int depth) {
        bool isEDB = true;
        bool found = false;
        ...
        if (!found) {
            return std::unique_ptr<tree_node>(new leaf_node("Relation " + relName + " not found"));
        }
    ...
    std::unique_ptr<tree_node> explain(std::string relName, elements tuple_elements) {
        ...
        if (lab == -1) {
            return std::unique_ptr<tree_node>(new leaf_node("Tuple " + relName + tuple_elements.getRepresentation() + " not found"));
        }
...
public:
    ...
    std::unique_ptr<tree_node> getTree(std::string relName, elements tuple_elements) {
        return std::move(explain(relName, tuple_elements));
    }

    std::unique_ptr<tree_node> getTree(std::string relName, plabel label) {
    ...
    void explain() {
        ...
        while (1) {
            ...
            if (command[0] == "setdepth") {
                ...
                std::unique_ptr<tree_node> t = ProvenanceTree(prog, depthLimit).getTree(query.first, tuple_elements);
                ...
                std::unique_ptr<tree_node> t = ProvenanceTree(prog, depthLimit).getTree(query.first, label);
                printTree(std::move(t));
