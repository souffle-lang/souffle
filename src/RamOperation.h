...
public:
    RamScan(std::unique_ptr<RamRelationRef> r, std::unique_ptr<RamOperation> nested, bool pureExistenceCheck,
            std::string profileText = "")
    ...
    RamScan* clone() const override {
        RamScan* res = new RamScan(std::unique_ptr<RamRelationRef>(relation->clone()),
...
public:
    RamAggregate(std::unique_ptr<RamOperation> nested, Function fun, std::unique_ptr<RamValue> value,
            std::unique_ptr<RamRelationRef> rel)
...
protected:
    ...
    // TODO (#541): rename
    std::unique_ptr<RamRelationRef> filter;

    /* Values for projection */
    std::vector<std::unique_ptr<RamValue>> values;

public:
    RamProject(std::unique_ptr<RamRelationRef> rel, size_t level)
            : RamOperation(RN_Project, level), relation(std::move(rel)), filter(nullptr) {}

    RamProject(std::unique_ptr<RamRelationRef> rel, const RamRelationRef& filter, size_t level)
            : RamOperation(RN_Project, level), relation(std::move(rel)),
              filter(std::make_unique<RamRelationRef>(filter)) {}
    ...
    /** Get filter */
    const RamRelationRef& getFilter() const {
        assert(hasFilter());
        return *filter;
    }
    ...
    RamProject* clone() const override {
        RamProject* res = new RamProject(std::unique_ptr<RamRelationRef>(relation->clone()), level);
        if (filter != nullptr) {
            res->filter = std::make_unique<RamRelationRef>(*filter);
        }
        for (auto& cur : values) {
