...
public:
    RamNotExists(std::unique_ptr<RamRelationRef> rel)
    ...
    RamNotExists* clone() const override {
        RamNotExists* res = new RamNotExists(std::unique_ptr<RamRelationRef>(relation->clone()));
        for (auto& cur : values) {
...
public:
    RamProvenanceNotExists(std::unique_ptr<RamRelationRef> rel)
    ...
    RamProvenanceNotExists* clone() const override {
        RamProvenanceNotExists* res =
                new RamProvenanceNotExists(std::unique_ptr<RamRelationRef>(relation->clone()));
        for (auto& cur : values) {
            RamValue* val = nullptr;
            if (cur != nullptr) {
                val = cur->clone();
...
public:
    RamEmpty(std::unique_ptr<RamRelationRef> rel) : RamCondition(RN_Empty), relation(std::move(rel)) {}
    ...
    RamEmpty* clone() const override {
        RamEmpty* res = new RamEmpty(std::unique_ptr<RamRelationRef>(relation->clone()));
        return res;
    }
